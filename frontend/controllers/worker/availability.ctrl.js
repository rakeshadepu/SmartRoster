 ...
 */
angular.module("TimetableApp").controller("AvailabilityCtrl", [
  "$scope",
  "$routeParams",
  "AuthService",
  "ApiService",
  function ($scope, $routeParams, AuthService, ApiService) {
    ...
    function loadExisting() {
      ApiService.getAvailability({ week_start: $scope.weekStart }).then(
        function (res) {
          $scope.existing = res.data.availability || [];
          $scope.slots.forEach(function (s) {
            s.checked = false;
            s.start_time = "09:00";
            delete s.id;
          });
          $scope.existing.forEach(function (a) {
            ...
            if (slot) {
              slot.checked = true;
              slot.start_time = a.start_time.slice(0, 5);
              slot.id = a.id;
            }
          });
        },
    ...
    $scope.submit = function () {
      ...
      $scope.error = null;
      $scope.success = null;
      var checked = $scope.slots.filter(function (s) {
        return s.checked;
      });
      if (checked.length === 0) {
      ...
      Promise.all(deletePromises)
        ...
        .then(function () {
          $scope.$apply(function () {
            $scope.success =
              "Availability saved for week of " + $scope.weekStart + ".";
            loadExisting();
          });
        })
        .catch(function () {
          $scope.$apply(function () {
            $scope.error = "Submission failed. Try again.";
          });
        })
        .finally(function () {
          $scope.$apply(function () {
